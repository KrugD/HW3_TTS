...
class CustomDirDataset(Dataset):
    ...
    Supports two directory layouts:
      1. Flat: audio_dir contains .wav files directly.
      2. Structured (HW3 format):
            root_dir/
            ├── audio/
            │   ├── file1.wav
            │   └── ...
    ...
    In resynthesis mode the dataset extracts mel-spectrograms from
    ground-truth audio.  Transcription texts are loaded when available
    and returned in the ``text`` field (useful for Full TTS pipeline).

    Args:
        audio_dir: Path to root directory (or to a folder with .wav files).
        mel_spec_transform: MelSpectrogram transform instance.
        target_sr: Target sample rate (default: 22050).
        resynthesize: If True, extract mel from ground-truth audio.
        mel_dir: Path to pre-computed mel-spectrograms (when resynthesize=False).
    ...
    def __init__(
        ...
        self.root_dir = Path(audio_dir)
        self.target_sr = target_sr
        self.resynthesize = resynthesize
        self.audio_extension = audio_extension

        # Detect directory layout: structured (audio/ subfolder) or flat
        if (self.root_dir / "audio").exists():
            self.audio_dir = self.root_dir / "audio"
        else:
            self.audio_dir = self.root_dir

        # Detect transcriptions directory
        self.transcriptions_dir: Optional[Path] = None
        if (self.root_dir / "transcriptions").exists():
            self.transcriptions_dir = self.root_dir / "transcriptions"
        ...
        n_texts = self._count_transcriptions()
        print(
            f"CustomDirDataset: {len(self.audio_paths)} audio samples, "
            f"{n_texts} transcriptions, resynthesize={resynthesize}"
        )
    ...
    def _count_transcriptions(self) -> int:
        """Count the number of transcription files that match audio files."""
        if self.transcriptions_dir is None:
            return 0
        count = 0
        for audio_path in self.audio_paths:
            txt_path = self.transcriptions_dir / f"{audio_path.stem}.txt"
            if txt_path.exists():
                count += 1
        return count

    def _load_transcription(self, file_name: str) -> Optional[str]:
        """Load transcription text for a given audio file name (without extension)."""
        if self.transcriptions_dir is None:
            return None
        txt_path = self.transcriptions_dir / f"{file_name}.txt"
        if txt_path.exists():
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing:
                ...
                - text: Transcription string (or None if unavailable).
        """
            ...
            if self.mel_dir is not None:
                ...
                if mel_path.exists():
                    ...
                    if mel_path.exists():
                        ...
                        print(
                            f"Warning: Pre-computed mel not found for {file_name}, "
        ...
        text = self._load_transcription(file_name)
