...
### 7.2 Анализ на внешних данных (MOS)

**Методология:**
- Использованы 3 ground-truth аудиофайла для MOS-оценки из [Google Drive](https://drive.google.com/drive/folders/1oClqdJjEGq6nDl9pAhqXivXGD5uJw1RJ)
...
**Количественные метрики:**
- **Mel L1 Distance**: 0.3958 +/- 0.0528

**Сравнение RUSLAN vs External:**

| Метрика | RUSLAN | External | Разница |
|---------|--------|----------|---------|
| Mel L1 Mean | 0.2680 | 0.3958 | +48% |
| Mel L1 Std | 0.0100 | 0.0528 | +5.3x |

**Ответы на вопросы:**

1. **Справедливы ли выводы из анализа на обучающих данных?**
   
   В целом да. На RUSLAN модель показывает стабильно низкую ошибку (0.27), на external MOS данных ошибка выше (0.40), но все еще приемлемая. Качество синтеза остается высоким.

2. **Какие различия вы видите между обучающим и внешним датасетами?**
   
   External имеет на 48% выше Mel L1 и в 5.3 раза выше дисперсию. Модель менее стабильна на unseen данных, но разброс значительно меньше, чем в предыдущих экспериментах с Silero TTS, так как MOS аудио записано в более близких акустических условиях.

3. **Объясните причины различий, если они есть**
   
   - Модель обучалась на RUSLAN (один спикер, определенные акустические условия)
   - MOS аудиофайлы имеют другие акустические характеристики и, возможно, другого спикера
   - Это типичное поведение domain gap - модель лучше работает на данных, похожих на обучающие
   - Тем не менее, разница умеренная (48%), что свидетельствует о хорошей генерализации вокодера
...
- На внешних данных (MOS): Mel L1 = 0.40, качество несколько ниже, но остается приемлемым
...
**Ограничения системы:**
- Domain gap: модель лучше работает на данных, похожих на обучающие (Mel L1: 0.27 vs 0.40)
- Границы сегментов: небольшие артефакты на концах waveform
- Дисперсия на unseen данных выше (0.053 vs 0.010), но не критически
